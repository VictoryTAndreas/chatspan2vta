...
def start_automation(file_path, start_date, end_date, satellite="sentinel2", cloud_percentage=15, indices=["NDVI", "NDWI", "SAVI", "EVI"]):
    ...
    # Initialize GEE
    try:
        # ✅ Directly set your service account email
        service_account = 'geospatial-analytics-services@geospatial-analytics-474609.iam.gserviceaccount.com'

        # ✅ Use raw string to avoid path errors (or use forward slashes)
        with open('geeservices.json', 'r') as f:
            private_key = f.read()
